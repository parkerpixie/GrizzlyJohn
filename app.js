(() => {
  ...
  function setupPodcasts() {
    ...
    const artworkApi = window.GrizzlyJohnPodcastArtwork;
    $('#podcastList').innerHTML = GRIZZLY_DATA.podcasts.map(podcast => {
      const artwork = artworkApi?.artworkFor(podcast) || { src: '', alt: `${podcast.title} podcast cover`, fallback: '🎙️' };
      const image = artwork.src ? `<img class="podcast-artwork-image" src="${escapeHtml(artwork.src)}" alt="${escapeHtml(artwork.alt)}" width="600" height="600">` : '';
      return `<article class="card podcast-card" data-podcast-id="${escapeHtml(podcast.id)}"><div class="podcast-art ${image ? '' : 'is-fallback'}">${image}<span class="podcast-art-fallback" aria-hidden="true">${artwork.fallback}</span></div><div class="podcast-copy"><h2>${escapeHtml(podcast.title)}</h2><p>${escapeHtml(podcast.description)}</p><div class="podcast-links"><a href="${podcast.spotify}" target="_blank" rel="noopener noreferrer" aria-label="Open ${escapeHtml(podcast.title)} on Spotify">Spotify ↗</a><a href="${podcast.apple}" target="_blank" rel="noopener noreferrer" aria-label="Open ${escapeHtml(podcast.title)} on Apple Podcasts">Apple ↗</a><a href="${podcast.amazon}" target="_blank" rel="noopener noreferrer" aria-label="Open ${escapeHtml(podcast.title)} on Amazon Music">Amazon ↗</a></div><button class="button button-secondary" type="button" data-log-podcast="${podcast.id}" aria-label="Log listening to ${escapeHtml(podcast.title)}">I listened to something</button></div></article>`;
    }).join('');
    $$('.podcast-artwork-image').forEach(image => image.addEventListener('error', () => artworkApi?.applyArtworkFallback(image), { once: true }));
